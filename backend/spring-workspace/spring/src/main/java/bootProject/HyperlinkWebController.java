...
public class HyperlinkWebController{
    ...
    public String addHyperlinkForm(Model model) {
    	model.addAttribute("hyperlink", new Hyperlink((long)0, null, null, null));
    ...
    public String addTagSubmit(@ModelAttribute MetaTag tag, Model model) {
    	// CHECK DATABASE SIZE TO KEEP IT LIMITED AND AVOID ATTACKS
    	long id = tag.getLinkId();
    	if (dataBase.containsKey(id)) {
    		model.addAttribute("error", 0);
    		Hyperlink hyperlink = dataBase.get(tag.getLinkId());
    		hyperlink.addTag(tag.getName());
    		dataBase.put(tag.getLinkId(), hyperlink);
    		model.addAttribute("hyperlink",	dataBase.get(tag.getLinkId()));
    		model.addAttribute("exists", true);
    ...
    public String addCommentForm(@PathVariable("id") long id, Model model) {
    	if (dataBase.containsKey(id)) {
    		...
    		comment.setId(0);
    		comment.setLinkId(hyperlink.getId());
    ...
    public String addCommentSubmit(@ModelAttribute Comment comment, Model model) {
    	// CHECK DATABASE SIZE TO KEEP IT LIMITED AND AVOID ATTACKS
    	long id = comment.getLinkId();
    	if (dataBase.containsKey(id)) {
    		model.addAttribute("error", 0);
    		Hyperlink hyperlink = dataBase.get(comment.getLinkId());
    		hyperlink.addComment(comment.getName());
    		dataBase.put(comment.getLinkId(), hyperlink);
    		model.addAttribute("hyperlink",	dataBase.get(comment.getLinkId()));
    		model.addAttribute("exists", true);
    		return "redirect:/show/" + id;
    	}
    	else {
